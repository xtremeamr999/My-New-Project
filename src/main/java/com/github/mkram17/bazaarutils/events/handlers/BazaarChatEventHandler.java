...
import com.github.mkram17.bazaarutils.features.OrderLimitVisual;
...
//        SoundUtil.notifyMultipleTimes(4);
    ...
    private static void onOrderCreated(BazaarChatEvent<? extends OrderInfo> event) {
        ...
        BUConfig.get().feature.orderLimitVisual.addOrderToLimit(order.getVolume()* order.getPricePerItem());
    ...
    private static void onInstaSell(BazaarChatEvent<? extends OrderInfo> event) {
        ...
        double totalPriceWithTax = totalPriceBeforeTax * ((100 + BUConfig.get().general.userBazaarTax)/100);

        //order limit does not count the tax
        BUConfig.get().feature.orderLimitVisual.addOrderToLimit(totalPriceBeforeTax);
    ...
    private static void onInstaBuy(BazaarChatEvent<? extends OrderInfo> event) {
        ...
        BUConfig.get().feature.orderLimitVisual.addOrderToLimit(totalPrice);
